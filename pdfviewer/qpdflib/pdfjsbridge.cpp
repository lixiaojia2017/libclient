...
void PdfJsBridge::jsGetBookMarkPage(int num, bool set)
{
  auto ptr = dynamic_cast<QPdfWidget*>(this->parent());
  if(ptr)
    {
      if(set)
        {
          ptr->bookmark.insert(num);
        }
      else
        {
          auto fd = ptr->bookmark.find(num);
          if(fd!=ptr->bookmark.end())
            {
              ptr->bookmark.erase(fd);
            }
        }
    }
}
...
void BridgeObject::jsGetBookMarkPage(int num,bool set)
{
    m_bridgePtr->jsGetBookMarkPage(num,set);
}
