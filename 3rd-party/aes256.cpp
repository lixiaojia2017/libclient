...
void copyStringToChar(std::string& sInput, int iNum, int iPos, char* pcArray) {
	if (iNum > sInput.size() || iPos > sInput.size()) throw std::out_of_range("Out of range");
	if (pcArray == nullptr) throw std::invalid_argument("Wrong pointer to array");
	for (int i = iPos; i < iNum + iPos; i++)
	{
		pcArray[i - iPos] = sInput.at(i);
	}
}

std::string sAes256Encrypt(std::string& sInput, std::string& sKey){
    //partition string (each part of 15 characters)
    int iPartCounter = sInput.size()/BLOCK;
    char** pcInput;
    int iNumberOfRest = sInput.size()% BLOCK;
        bool bRest = iNumberOfRest > 0;

	if (bRest) {
		pcInput = new char *[iPartCounter + 1];
		for (int i = 0; i<iPartCounter+1; i++) {
    ...
    //copying to array (full blocks)
    for(int i =0;i<iPartCounter;i++){
                copyStringToChar(sInput, BLOCK, i * BLOCK, pcInput[i]);
    }
        //rest
        if (bRest) {
                copyStringToChar(sInput, iNumberOfRest, iPartCounter * BLOCK, pcInput[iPartCounter]);
                //add suplements
                for (int i = iNumberOfRest; i<BLOCK; i++) {
                        pcInput[iPartCounter][i] = supplement;
                }
        }
     std::string sResult;
	 ...
	 if (bRest)
	 {
		 for (int i = 0; i<iPartCounter+1; i++) {
			 ...
			 aes256_encrypt_ecb(&ctx, (unsigned char*)pcInput[i]);
			 //append to string
...
std::string sAes256Decrypt(std::string& sInput,std::string& sKey){
